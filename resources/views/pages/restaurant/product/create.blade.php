...
@section('content')
    <div class="container">
        ...
        {{-- Actual Edit Form --}}
        <form method="POST" action="{{route('restaurant.product.store', $restaurant)}}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input type="number" class="form-control @error('price') is-invalid @enderror" name="price" id="price" value="{{old('price', 0)}}" min="0" required>
                @error('price')
                    <small class="form-text text-danger">{{$message}}</small>
                @enderror
            </div>
            <div class="form-group">
                ...
                <input type="range" class="form-control-range @error('discount') is-invalid @enderror" id="discountRange" name="discount" value="{{old('discount', 0)}}" min="0" max="100" required>
                @error('discount')
                    <small class="form-text text-danger">{{$message}}</small>
                @enderror
            </div>
            <div class="form-group"> {{-- Products --}}
                ...
                <select name="group" class="custom-select">
                    ...
                    @foreach ($groups as $group)
                        <option value="{{$group->name}}" @if(old('group') == $group->name){{"selected"}}@endif>{{$group->name}}</option>
                    @endforeach
                </select>
                @error('group')
                    <small class="form-text text-danger">{{$message}}</small>
            ...
            <div class="form-check">
                <input class="form-check-input" type="checkbox" name="available" value="1" checked> {{-- "1" will be converted to true in backend --}}
