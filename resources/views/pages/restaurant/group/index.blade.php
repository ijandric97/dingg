...
@endif
    ...
    <h1>Product Groups</h1> {{-- Title --}}
    <p>Products are organised into groups. You can add/edit/remove groups. To put products into a created group go to the Products page.</p>
        ...
        @for ($i = 0; $i < count(old('id', $groups)); $i++)
            <div class="list-group-item border mb-2">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('description.'.$i) is-invalid @enderror" name="description[]"
                ...
                <input type="number" name="id[]" value="{{old('id.'.$i, $i < count($groups) ? $groups[$i]->id : '')}}" hidden>
            </div>
        @endfor
        </div>
    </form>
